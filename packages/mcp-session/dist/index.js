...
function createServer() {
  const server = new import_mcp.McpServer({ name: SERVER_NAME, version: "0.1.0" });
  server.registerTool(
    "devpilot_session_join",
    {
      ...
      inputSchema: {
        url: import_zod.z.string().describe("The full join link, including the #k=\u2026 fragment. Without the fragment there is no key."),
        displayName: import_zod.z.string().optional().describe('How this agent appears in the transcript. Defaults to "Claude Code".')
      }
    },
    async ({ url, displayName }) => {
      try {
        client = await import_bridge_client.SharedSessionClient.join({
          link: url,
