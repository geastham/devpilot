...
__export(index_exports, {
  ...
  SharedSessionClient: () => SharedSessionClient
});
...
var BridgeClient = class {
  ...
  async request(path, init = {}) {
    ...
    if (!res.ok) {
      ...
      throw new BridgeError(
        (0, import_bridge_protocol.formatApiError)(
          body,
  ...
  async register(capabilities) {
    const body = import_bridge_protocol.RegisterRequestSchema.parse(capabilities);
    const raw = await this.request("/api/orchestrators/register", {
      method: "POST",
      body: JSON.stringify(body)
    });
    const parsed = import_bridge_protocol.RegisterResponseSchema.parse(raw);
  ...
  async adoptSessions(request) {
    const body = import_bridge_protocol.AdoptionRequestSchema.parse(request);
    const raw = await this.request("/api/adoptions", {
      method: "POST",
      body: JSON.stringify(body)
    });
    return import_bridge_protocol.AdoptionResponseSchema.parse(raw);
  }
  /**
  ...
  async reportDiscovery(request) {
    try {
      const body = import_bridge_protocol.DiscoveryRequestSchema.parse(request);
      const raw = await this.request("/api/discovery", {
        method: "POST",
        body: JSON.stringify(body)
      });
      return import_bridge_protocol.DiscoveryResponseSchema.parse(raw);
    } catch {
      return null;
    }
  }
   ...
   */
  async streamEvents(sessionId, events) {
...
var _SharedSessionClient = class _SharedSessionClient {
  ...
  static async join(options) {
    var _a;
    const { sessionId, key } = (0, import_bridge_protocol2.parseJoinLink)(options.link);
  ...
  async post(text, opts = {}) {
    const ciphertext = await import_bridge_protocol2.sessionCrypto.encrypt(text, __privateGet(this, _key));
  ...
  async who() {
    const { verifier } = await import_bridge_protocol2.sessionCrypto.deriveJoinCredentials(__privateGet(this, _key));
    const res = await __privateGet(this, _fetchImpl).call(this, `${this.baseUrl}/api/sessions/shared/${this.sessionId}`, {
      headers: { [import_bridge_protocol2.JOIN_PROOF_HEADER]: verifier }
    });
    if (!res.ok) {
      const body2 = await res.json().catch(() => null);
      throw new BridgeError((0, import_bridge_protocol2.formatApiError)(body2, `Roster failed: ${res.status}`), res.status);
    }
...
requestJoin_fn = async function(fetchImpl, baseUrl, sessionId, key, joinOptions, resumeToken) {
  ...
  if (!res.ok) {
    ...
    throw new BridgeError((0, import_bridge_protocol2.formatApiError)(body2, `Join failed: ${res.status}`), res.status);
  }
  const body = await res.json();
  return {
...
request_fn = async function(path, init = {}, retried = false) {
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => null);
    throw new BridgeError(
      (0, import_bridge_protocol2.formatApiError)(body, `${init.method ?? "GET"} ${path} failed: ${res.status}`),
      res.status
    );
  }
...
decode_fn = async function(m) {
  ...
  try {
    return { ...base, text: await import_bridge_protocol2.sessionCrypto.decrypt(m.ciphertext, __privateGet(this, _key)), status: "ok" };
  } catch {
    return { ...base, text: null, status: "undecryptable" };
  }
};
