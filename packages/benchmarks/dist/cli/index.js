...
var WorkspaceManager = class {
  ...
  async createWorkspace(config) {
    ...
    const workspacePath = (0, import_path.join)(TEMP_BASE, runId, scenario, benchmarkId);
    await (0, import_promises.mkdir)(workspacePath, { recursive: true });
    ...
    const prdSource = (0, import_path.join)(benchmarkPath, "PRD.md");
    const prdDest = (0, import_path.join)(workspacePath, "PRD.md");
    try {
      await (0, import_promises.cp)(prdSource, prdDest);
    } catch {
      throw new Error(`PRD.md not found at ${prdSource}`);
    }
    const fixturesSource = (0, import_path.join)(benchmarkPath, "fixtures");
    const fixturesDest = (0, import_path.join)(workspacePath, "fixtures");
    try {
      const fixturesStat = await (0, import_promises.stat)(fixturesSource);
      if (fixturesStat.isDirectory()) {
        await (0, import_promises.cp)(fixturesSource, fixturesDest, { recursive: true });
      }
    } catch {
    }
    const acceptanceSource = (0, import_path.join)(benchmarkPath, "acceptance");
    const acceptanceDest = (0, import_path.join)(workspacePath, "acceptance");
    try {
      const acceptanceStat = await (0, import_promises.stat)(acceptanceSource);
      if (acceptanceStat.isDirectory()) {
        await (0, import_promises.cp)(acceptanceSource, acceptanceDest, { recursive: true });
  ...
  async findBenchmarkPath(benchmarksDir, benchmarkId) {
    const exactPath = (0, import_path.join)(benchmarksDir, benchmarkId);
    try {
      const s = await (0, import_promises.stat)(exactPath);
      if (s.isDirectory()) return exactPath;
    } catch {
    }
    try {
      const entries = await (0, import_promises.readdir)(benchmarksDir);
      const prefix = benchmarkId.split("-")[0];
      for (const entry of entries) {
        if (entry.startsWith(`${prefix}-`)) {
          const fullPath = (0, import_path.join)(benchmarksDir, entry);
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.endsWith(".sh")) {
          const scriptPath = (0, import_path.join)(dir, entry);
  ...
  async initializePackageJson(workspacePath, benchmarkId, benchmarkPath) {
    const templatePath = (0, import_path.join)(benchmarkPath, "package.template.json");
    try {
      const template = await (0, import_promises.readFile)(templatePath, "utf-8");
      await (0, import_promises.writeFile)((0, import_path.join)(workspacePath, "package.json"), template);
      return;
    } catch {
    }
    const packageJson = {
    ...
    await (0, import_promises.writeFile)(
      (0, import_path.join)(workspacePath, "package.json"),
  ...
  async archiveWorkspace(workspacePath, archivePath) {
    await (0, import_promises.mkdir)((0, import_path.dirname)(archivePath), { recursive: true });
    try {
      await tar.create(
        {
          gzip: true,
          file: archivePath,
          cwd: (0, import_path.dirname)(workspacePath)
        },
      ...
      await this.processManager.spawn("tar", [
        ...
        archivePath,
        "-C",
        (0, import_path.dirname)(workspacePath),
        workspacePath.split("/").pop()
      ]);
    }
  }
...
var AcceptanceRunner = class {
  ...
  async findTestScript(workspacePath) {
    ...
    for (const candidate of candidates) {
      const fullPath = (0, import_path2.join)(workspacePath, candidate);
      try {
  ...
  async needsServer(workspacePath, testScript) {
    try {
      const content = await (0, import_promises2.readFile)(testScript, "utf-8");
  ...
  async startServer(workspacePath) {
    ...
    for (const { cmd, args } of serverCommands) {
      try {
        if (cmd === "node") {
          await (0, import_promises2.stat)((0, import_path2.join)(workspacePath, args[0]));
        }
...
var BenchmarkIdSchema = import_zod.z.enum([
  "01-forgepress",
  "02-taskforge",
  "03-insightboard"
]);
var ScenarioTypeSchema = import_zod.z.enum(["baseline", "devpilot"]);
var ModelTierSchema = import_zod.z.enum(["haiku", "sonnet", "opus"]);
var ModelPricingSchema = import_zod.z.object({
  model: import_zod.z.string(),
  inputPer1M: import_zod.z.number().positive(),
  outputPer1M: import_zod.z.number().positive(),
  cacheReadPer1M: import_zod.z.number().nonnegative(),
  cacheWritePer1M: import_zod.z.number().nonnegative()
});
...
var BenchmarkConfigSchema = import_zod.z.object({
  // Benchmark selection
  benchmarks: import_zod.z.object({
    /** Benchmark IDs to include */
    include: import_zod.z.array(BenchmarkIdSchema).default([
      "01-forgepress",
      "02-taskforge",
      "03-insightboard"
    ]),
    /** Path to benchmark definitions */
    benchmarkDir: import_zod.z.string().default("./benchmarks"),
    /** Path to ground truth files */
    groundTruthDir: import_zod.z.string().default("./benchmarks/ground-truth")
  }).default({}),
  // Baseline scenario settings
  baseline: import_zod.z.object({
    /** Path to Claude Code CLI binary */
    claudeCodeBinary: import_zod.z.string().default("claude"),
    /** Model to use */
    model: ModelTierSchema.default("sonnet"),
    /** Maximum retries on failure */
    maxRetries: import_zod.z.number().int().nonnegative().default(1)
  }).default({}),
  // DevPilot scenario settings
  devpilot: import_zod.z.object({
    /** Orchestrator mode */
    orchestratorMode: import_zod.z.enum(["http", "ao-cli"]).default("ao-cli"),
    /** HTTP orchestrator URL (for http mode) */
    orchestratorUrl: import_zod.z.string().url().optional(),
    /** API key for orchestrator */
    apiKey: import_zod.z.string().optional(),
    /** Max concurrent sessions */
    maxConcurrentSessions: import_zod.z.number().int().positive().default(4),
    /** Model for planning */
    ...
    aoProjectName: import_zod.z.string().optional()
  }).default({}),
  // Execution settings
  execution: import_zod.z.object({
    /** Timeout per scenario in minutes */
    timeoutMinutes: import_zod.z.number().positive().default(10),
    /** Ideal completion time in minutes (for scoring) */
    idealTimeMinutes: import_zod.z.number().positive().default(5),
    /** Number of iterations per scenario */
    iterations: import_zod.z.number().int().positive().default(1),
    /** Run benchmarks in parallel */
    parallel: import_zod.z.boolean().default(false),
    /** Archive workspaces after execution */
    archiveWorkspaces: import_zod.z.boolean().default(true)
  }).default({}),
  // Pricing configuration
  pricing: import_zod.z.array(ModelPricingSchema).default(DEFAULT_MODEL_PRICING),
  // Scoring weights
  scoring: import_zod.z.object({
    weights: import_zod.z.object({
      acceptanceTests: import_zod.z.number().min(0).max(1).default(0.3),
      wavePlanQuality: import_zod.z.number().min(0).max(1).default(0.25),
      firstAttemptPassRate: import_zod.z.number().min(0).max(1).default(0.2),
      completionTime: import_zod.z.number().min(0).max(1).default(0.15),
      reworkRatio: import_zod.z.number().min(0).max(1).default(0.1)
    }).refine(
      (weights) => {
  ...
  output: import_zod.z.object({
    /** Results directory */
    resultsDir: import_zod.z.string().default("./benchmarks/results"),
    /** Reporters to use */
    reporters: import_zod.z.array(import_zod.z.enum(["console", "markdown", "json"])).default([
      "console",
      "markdown",
      "json"
    ])
...
var RunConfigSchema = import_zod.z.object({
  benchmarks: import_zod.z.array(BenchmarkIdSchema),
  scenarios: import_zod.z.array(ScenarioTypeSchema),
  iterations: import_zod.z.number().int().positive(),
  parallel: import_zod.z.boolean(),
  timeoutMs: import_zod.z.number().int().positive(),
  archiveWorkspaces: import_zod.z.boolean(),
  outputDir: import_zod.z.string(),
  versionTag: import_zod.z.string().optional(),
  maxConcurrentSessions: import_zod.z.number().int().positive(),
...
function getGitCommit(projectRoot = process.cwd()) {
  try {
    return (0, import_child_process2.execSync)("git rev-parse HEAD", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
function getGitBranch(projectRoot = process.cwd()) {
  try {
    return (0, import_child_process2.execSync)("git rev-parse --abbrev-ref HEAD", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
function getGitTag(projectRoot = process.cwd()) {
  try {
    const tag = (0, import_child_process2.execSync)("git describe --exact-match --tags HEAD", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
function getGitDescribe(projectRoot = process.cwd()) {
  try {
    return (0, import_child_process2.execSync)("git describe --tags --always", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
var BenchmarkRunner = class {
   ...
   */
  async run(runConfig) {
    const runId = (0, import_cuid23.createId)();
  ...
  async runBenchmark(runId, benchmarkId, runConfig, groundTruth) {
    ...
    if (runConfig.scenarios.includes("baseline")) {
      ...
      await this.workspaceManager.cleanupWorkspace(workspaceId, {
        ...
        archivePath: (0, import_path5.join)(
  ...
  async loadGroundTruths(benchmarkIds) {
    ...
    const groundTruthDir = (0, import_path5.join)(this.config.benchmarksDir, "ground-truth");
    for (const benchmarkId of benchmarkIds) {
      try {
        const exactPath = (0, import_path5.join)(groundTruthDir, `${benchmarkId}.json`);
        const content = await (0, import_promises5.readFile)(exactPath, "utf-8");
        ...
        try {
          ...
          if (matchingFile) {
            const content = await (0, import_promises5.readFile)((0, import_path5.join)(groundTruthDir, matchingFile), "utf-8");
            groundTruths.set(benchmarkId, JSON.parse(content));
          }
        } catch {
        }
...
var ConsoleReporter = class {
  ...
  printHeader(run) {
    ...
    console.log(import_chalk.default.bold.blue("\u2551") + import_chalk.default.bold.white("            DevPilot Benchmark Suite                        ") + import_chalk.default.bold.blue("\u2551"));
    console.log(import_chalk.default.bold.blue("\u255A\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255D"));
    console.log("");
    console.log(`  ${import_chalk.default.gray("Version:")}    ${run.version}`);
    console.log(`  ${import_chalk.default.gray("Run ID:")}     ${run.id}`);
    console.log(`  ${import_chalk.default.gray("Timestamp:")}  ${run.timestamp}`);
    console.log("");
  }
  /**
   * Print run summary.
  ...
  printSummary(run) {
    ...
    const speedupStr = summary.avgSpeedup > 1 ? import_chalk.default.green(`${summary.avgSpeedup.toFixed(2)}x faster`) : summary.avgSpeedup < 1 ? import_chalk.default.red(`${(1 / summary.avgSpeedup).toFixed(2)}x slower`) : import_chalk.default.gray("no change");
    console.log(
      BOX_CHARS.vertical + ` Avg Speedup: ${speedupStr.padEnd(44)}` + BOX_CHARS.vertical
    );
    const costStr = summary.avgCostReduction > 0 ? import_chalk.default.green(`${(summary.avgCostReduction * 100).toFixed(1)}% reduction`) : summary.avgCostReduction < 0 ? import_chalk.default.red(`${(-summary.avgCostReduction * 100).toFixed(1)}% increase`) : import_chalk.default.gray("no change");
    console.log(
      BOX_CHARS.vertical + ` Avg Cost: ${costStr.padEnd(47)}` + BOX_CHARS.vertical
    );
    const qualityStr = summary.avgQualityDelta > 0 ? import_chalk.default.green(`+${(summary.avgQualityDelta * 100).toFixed(1)}%`) : summary.avgQualityDelta < 0 ? import_chalk.default.red(`${(summary.avgQualityDelta * 100).toFixed(1)}%`) : import_chalk.default.gray("no change");
    console.log(
  ...
  printResultsTable(run) {
    ...
    for (const result of run.benchmarks) {
      ...
      const quality = result.comparison ? this.formatPercentage(result.comparison.qualityDelta, true) : import_chalk.default.gray("-".padEnd(10));
      const wave = result.waveAnalysis ? this.formatWaveScore(result.waveAnalysis.score) : import_chalk.default.gray("-".padEnd(8));
...
${benchmarkId} Comparison`));
    ...
    if (waveAnalysis) {
      console.log("");
      console.log(import_chalk.default.bold("  Wave Plan Analysis"));
...
${scenario.scenario.toUpperCase()} Score Card`));
    ...
    const gradeColor = score.grade === "A" ? import_chalk.default.green : score.grade === "B" ? import_chalk.default.blue : score.grade === "C" ? import_chalk.default.yellow : score.grade === "D" ? import_chalk.default.magenta : import_chalk.default.red;
  ...
  printScoreBar(label, score, weight) {
    ...
    const color = score >= 80 ? import_chalk.default.green : score >= 60 ? import_chalk.default.yellow : import_chalk.default.red;
    const bar = color("\u2588".repeat(filledWidth)) + import_chalk.default.gray("\u2591".repeat(emptyWidth));
    const weightStr = `(${(weight * 100).toFixed(0)}%)`.padEnd(6);
    console.log(`  ${label.padEnd(14)} ${bar} ${score.toFixed(1).padStart(5)}/100 ${import_chalk.default.gray(weightStr)}`);
  }
  /**
...
${analysis.benchmarkId} Trends`));
  ...
  createProgressBar(percent, width) {
    ...
    return import_chalk.default.blue("\u2588".repeat(filled)) + import_chalk.default.gray("\u2591".repeat(empty));
  }
  /**
  ...
  formatSpeedup(speedup) {
    ...
    if (speedup > 1.1) return import_chalk.default.green(str.padEnd(10));
    if (speedup < 0.9) return import_chalk.default.red(str.padEnd(10));
    return import_chalk.default.gray(str.padEnd(10));
  }
  /**
  ...
  formatPercentage(value, isGood) {
    ...
    if (isGood) {
      if (value > 0.05) return import_chalk.default.green(str.padEnd(10));
      if (value < -0.05) return import_chalk.default.red(str.padEnd(10));
    } else {
      if (value < -0.05) return import_chalk.default.green(str.padEnd(10));
      if (value > 0.05) return import_chalk.default.red(str.padEnd(10));
    }
  ...
  formatWaveScore(score) {
    const str = `${score}/25`;
    if (score >= 20) return import_chalk.default.green(str.padEnd(8));
    if (score >= 15) return import_chalk.default.blue(str.padEnd(8));
    if (score >= 10) return import_chalk.default.yellow(str.padEnd(8));
    return import_chalk.default.red(str.padEnd(8));
  }
  /**
   * Format duration.
   */
  formatDuration(ms) {
    if (ms < 0) {
      return import_chalk.default.red(`-${this.formatDurationAbs(-ms)}`);
    }
    return import_chalk.default.green(`+${this.formatDurationAbs(ms)}`);
  }
  ...
  formatTrendDirection(direction) {
    switch (direction) {
      case "improving":
        return import_chalk.default.green("\u2191 Improving");
      case "regressing":
        return import_chalk.default.red("\u2193 Regressing");
      case "stable":
        return import_chalk.default.gray("\u2192 Stable");
    }
  }
  /**
   * Format delta value.
  ...
  formatDelta(value, suffix) {
    ...
    if (value > 0) return import_chalk.default.green(str);
    if (value < 0) return import_chalk.default.red(str);
    return import_chalk.default.gray(str);
  }
};
...
// src/cli/run.ts
var runCommand = new import_commander.Command("run").description("Execute benchmark suite").option(
  "-b, --benchmarks <ids>",
  "Comma-separated benchmark IDs (default: all)",
  (val) => val.split(",").map((s) => s.trim())
).option(
  "-s, --scenarios <types>",
  "Scenarios to run: baseline,devpilot (default: both)",
  (val) => val.split(",").map((s) => s.trim())
).option("-n, --iterations <n>", "Iterations per scenario", parseInt, 1).option("-c, --concurrency <n>", "Max concurrent sessions", parseInt, 4).option("--parallel", "Run benchmarks in parallel").option("--timeout <minutes>", "Per-scenario timeout in minutes", parseInt, 10).option("--no-archive", "Skip workspace archival").option("--dry-run", "Validate config without running").option("-v, --verbose", "Verbose output").option("--config <path>", "Path to config file").option("--benchmarks-dir <path>", "Path to benchmarks directory").option("--results-dir <path>", "Path to results directory").action(async (options) => {
...
var JsonReporter = class {
  ...
  async writeReport(filePath, content) {
    await (0, import_promises6.mkdir)((0, import_path7.dirname)(filePath), { recursive: true });
...
var MarkdownReporter = class {
   ...
   */
  async writeReport(filePath, content) {
    await (0, import_promises7.mkdir)((0, import_path8.dirname)(filePath), { recursive: true });
...
// src/cli/report.ts
var reportCommand = new import_commander2.Command("report").description("Generate benchmark report").argument("[runId]", "Run ID to report on").option("--format <type>", "Output format: console, markdown, json", "console").option("--output <file>", "Output file path (for markdown/json)").option("--latest", "Use most recent run").option("--results-dir <path>", "Path to results directory").action(async (runId, options) => {
  const consoleReporter = createConsoleReporter();
  try {
    const resultsDir = options.resultsDir ?? (0, import_path9.join)(process.cwd(), "benchmarks", "results");
...
var compareCommand = new import_commander3.Command("compare").description("Compare benchmark results between versions").argument("<v1>", "First version or run ID").argument("<v2>", "Second version or run ID").option("-b, --benchmark <id>", "Specific benchmark to compare").option("--format <type>", "Output format: console, markdown, json", "console").option("--output <file>", "Output file path").option("--results-dir <path>", "Path to results directory").action(async (v1, v2, options) => {
  const consoleReporter = createConsoleReporter();
  try {
    const resultsDir = options.resultsDir ?? (0, import_path10.join)(process.cwd(), "benchmarks", "results");
    const reader = createHistoryReader(resultsDir);
    ...
    switch (options.format) {
      ...
      case "markdown": {
        ...
        for (const comp of comparisons) {
          ...
          const costDelta = comp.delta.costUsd >= 0 ? `+$${comp.delta.costUsd.toFixed(2)}` : `-$${Math.abs(comp.delta.costUsd).toFixed(2)}`;
          const qualityDelta = comp.delta.passRate >= 0 ? `+${(comp.delta.passRate * 100).toFixed(1)}%` : `${(comp.delta.passRate * 100).toFixed(1)}%`;
...
async function findRun(reader, versionOrId) {
  const versions = await reader.listVersions();
...
function formatDeltaDuration(ms) {
  const sign = ms >= 0 ? "+" : "-";
...
var historyCommand = new import_commander4.Command("history").description("View benchmark history and trends").argument("[benchmarkId]", "Specific benchmark to show history for").option("--last <n>", "Show last N versions", parseInt, 10).option("--metric <name>", "Highlight specific metric: speedup, cost, quality", "speedup").option("--format <type>", "Output format: console, json", "console").option("--output <file>", "Output file path").option("--results-dir <path>", "Path to results directory").action(async (benchmarkId, options) => {
  const consoleReporter = createConsoleReporter();
  try {
    const resultsDir = options.resultsDir ?? (0, import_path11.join)(process.cwd(), "benchmarks", "results");
    const reader = createHistoryReader(resultsDir);
    ...
    const benchmarkData = /* @__PURE__ */ new Map();
    for (const version of versions) {
      const runs = await reader.listRuns(version);
...
var listCommand = new import_commander5.Command("list").description("List benchmarks or runs").argument("[type]", "What to list: benchmarks, runs, versions", "benchmarks").option("--version <v>", "Filter runs by version").option("--results-dir <path>", "Path to results directory").option("--benchmarks-dir <path>", "Path to benchmarks directory").action(async (type, options) => {
  const consoleReporter = createConsoleReporter();
  try {
    const projectRoot = process.cwd();
    const benchmarksDir = options.benchmarksDir ?? (0, import_path12.join)(projectRoot, "benchmarks");
    const resultsDir = options.resultsDir ?? (0, import_path12.join)(benchmarksDir, "results");
    switch (type) {
      case "benchmarks": {
        console.log("");
        ...
        console.log("\u2550".repeat(50));
        console.log("");
        const entries = await (0, import_promises8.readdir)(benchmarksDir);
        for (const entry of entries) {
          if (entry.startsWith(".") || entry === "results" || entry === "ground-truth" || entry === "00-common") {
            continue;
          }
          const entryPath = (0, import_path12.join)(benchmarksDir, entry);
          const entryStat = await (0, import_promises8.stat)(entryPath);
          if (entryStat.isDirectory()) {
            try {
              const prdPath = (0, import_path12.join)(entryPath, "PRD.md");
              await (0, import_promises8.stat)(prdPath);
              const prdContent = await (0, import_promises8.readFile)(prdPath, "utf-8");
...
var validateCommand = new import_commander6.Command("validate").description("Validate benchmark configuration").argument("[benchmarkId]", "Specific benchmark to validate").option("--benchmarks-dir <path>", "Path to benchmarks directory").option("--fix", "Attempt to fix issues").action(async (benchmarkId, options) => {
  const consoleReporter = createConsoleReporter();
  try {
    const benchmarksDir = options.benchmarksDir ?? (0, import_path13.join)(process.cwd(), "benchmarks");
    const results = [];
    let benchmarks = [];
    if (benchmarkId) {
      benchmarks = [benchmarkId];
    } else {
      const entries = await (0, import_promises9.readdir)(benchmarksDir);
    ...
    console.log("\u2500".repeat(40));
    const groundTruthDir = (0, import_path13.join)(benchmarksDir, "ground-truth");
    try {
      const gtFiles = await (0, import_promises9.readdir)(groundTruthDir);
      for (const file of gtFiles) {
        if (!file.endsWith(".json")) continue;
        const filePath = (0, import_path13.join)(groundTruthDir, file);
        try {
...
async function validateBenchmark(benchmarksDir, benchmarkId) {
  const result = {
    benchmarkId,
    valid: true,
    errors: [],
  ...
  const benchmarkPath = (0, import_path13.join)(benchmarksDir, benchmarkId);
  try {
    const s = await (0, import_promises9.stat)(benchmarkPath);
    if (!s.isDirectory()) {
  ...
  const prdPath = (0, import_path13.join)(benchmarkPath, "PRD.md");
  try {
    await (0, import_promises9.stat)(prdPath);
  } catch {
    result.errors.push("PRD.md not found");
    result.valid = false;
  }
  try {
    const prdContent = await (0, import_promises9.readFile)(prdPath, "utf-8");
  ...
  const fixturesPath = (0, import_path13.join)(benchmarkPath, "fixtures");
  try {
    await (0, import_promises9.stat)(fixturesPath);
  } catch {
    result.warnings.push("No fixtures directory");
  }
  const acceptancePath = (0, import_path13.join)(benchmarkPath, "acceptance");
  try {
    await (0, import_promises9.stat)(acceptancePath);
    const testScripts = ["run-tests.sh", "acceptance.sh", "test.sh"];
    let hasScript = false;
    for (const script of testScripts) {
      try {
        const scriptPath = (0, import_path13.join)(acceptancePath, script);
        await (0, import_promises9.stat)(scriptPath);
        hasScript = true;
        try {
          await (0, import_promises9.access)(scriptPath, import_promises9.constants.X_OK);
        } catch {
...
// src/cli/index.ts
var benchCommand = new import_commander7.Command("bench").description("DevPilot Benchmark Suite").addCommand(runCommand).addCommand(reportCommand).addCommand(compareCommand).addCommand(historyCommand).addCommand(listCommand).addCommand(validateCommand);
