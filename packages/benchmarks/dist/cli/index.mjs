...
var AcceptanceRunner = class {
  ...
  async findTestScript(workspacePath) {
    ...
    for (const candidate of candidates) {
      const fullPath = join2(workspacePath, candidate);
      try {
  ...
  async needsServer(workspacePath, testScript) {
    try {
      const content = await readFile2(testScript, "utf-8");
  ...
  async startServer(workspacePath) {
    ...
    for (const { cmd, args } of serverCommands) {
      try {
        if (cmd === "node") {
          await stat2(join2(workspacePath, args[0]));
        }
...
var BaselineExecutor = class {
  ...
  async execute(workspace, collector) {
    ...
    try {
      const prdPath = join3(workspace.path, "PRD.md");
      const prdContent = await readFile3(prdPath, "utf-8");
...
var DevPilotExecutor = class {
  ...
  async execute(workspace, collector) {
    ...
    const errors = [];
    try {
      const prdPath = join4(workspace.path, "PRD.md");
...
var BenchmarkRunner = class {
  ...
  async run(runConfig) {
    ...
    try {
      ...
      await this.resultsWriter.updateLatestSymlink(
        join6(this.config.resultsDir, version.version, startTime.toISOString().replace(/[:.]/g, "-"))
      );
      return run;
    } finally {
  ...
  async loadGroundTruths(benchmarkIds) {
    ...
    const groundTruthDir = join6(this.config.benchmarksDir, "ground-truth");
    for (const benchmarkId of benchmarkIds) {
      try {
        const exactPath = join6(groundTruthDir, `${benchmarkId}.json`);
        const content = await readFile5(exactPath, "utf-8");
        ...
        try {
          ...
          if (matchingFile) {
            const content = await readFile5(join6(groundTruthDir, matchingFile), "utf-8");
            groundTruths.set(benchmarkId, JSON.parse(content));
          }
        } catch {
        }
...
// src/cli/run.ts
var runCommand = new Command("run").description("Execute benchmark suite").option(
  "-b, --benchmarks <ids>",
  ...
  try {
    ...
    const benchmarksDir = options.benchmarksDir ?? join7(projectRoot, "benchmarks");
    const resultsDir = options.resultsDir ?? join7(benchmarksDir, "results");
    let config = {};
    if (options.config) {
...
var compareCommand = new Command3("compare").description("Compare benchmark results between versions").argument("<v1>", "First version or run ID").argument("<v2>", "Second version or run ID").option("-b, --benchmark <id>", "Specific benchmark to compare").option("--format <type>", "Output format: console, markdown, json", "console").option("--output <file>", "Output file path").option("--results-dir <path>", "Path to results directory").action(async (v1, v2, options) => {
  const consoleReporter = createConsoleReporter();
  try {
    const resultsDir = options.resultsDir ?? join11(process.cwd(), "benchmarks", "results");
    const reader = createHistoryReader(resultsDir);
...
import { readdir as readdir5, stat as stat4, readFile as readFile7 } from "fs/promises";
var listCommand = new Command5("list").description("List benchmarks or runs").argument("[type]", "What to list: benchmarks, runs, versions", "benchmarks").option("--version <v>", "Filter runs by version").option("--results-dir <path>", "Path to results directory").option("--benchmarks-dir <path>", "Path to benchmarks directory").action(async (type, options) => {
  ...
  try {
    const projectRoot = process.cwd();
    const benchmarksDir = options.benchmarksDir ?? join13(projectRoot, "benchmarks");
    const resultsDir = options.resultsDir ?? join13(benchmarksDir, "results");
    switch (type) {
      case "benchmarks": {
        console.log("");
        ...
        console.log("\u2550".repeat(50));
        console.log("");
        const entries = await readdir5(benchmarksDir);
        for (const entry of entries) {
          if (entry.startsWith(".") || entry === "results" || entry === "ground-truth" || entry === "00-common") {
            continue;
          }
          const entryPath = join13(benchmarksDir, entry);
          const entryStat = await stat4(entryPath);
          if (entryStat.isDirectory()) {
            try {
              const prdPath = join13(entryPath, "PRD.md");
              await stat4(prdPath);
              const prdContent = await readFile7(prdPath, "utf-8");
...
import { readdir as readdir6, stat as stat5, readFile as readFile8, access as access2, constants } from "fs/promises";
var validateCommand = new Command6("validate").description("Validate benchmark configuration").argument("[benchmarkId]", "Specific benchmark to validate").option("--benchmarks-dir <path>", "Path to benchmarks directory").option("--fix", "Attempt to fix issues").action(async (benchmarkId, options) => {
  const consoleReporter = createConsoleReporter();
  try {
    const benchmarksDir = options.benchmarksDir ?? join14(process.cwd(), "benchmarks");
    const results = [];
    let benchmarks = [];
    if (benchmarkId) {
      benchmarks = [benchmarkId];
    } else {
      const entries = await readdir6(benchmarksDir);
    ...
    console.log("\u2500".repeat(40));
    const groundTruthDir = join14(benchmarksDir, "ground-truth");
    try {
      const gtFiles = await readdir6(groundTruthDir);
      for (const file of gtFiles) {
        if (!file.endsWith(".json")) continue;
        const filePath = join14(groundTruthDir, file);
        try {
...
async function validateBenchmark(benchmarksDir, benchmarkId) {
  ...
  const benchmarkPath = join14(benchmarksDir, benchmarkId);
  try {
    const s = await stat5(benchmarkPath);
    if (!s.isDirectory()) {
  ...
  const prdPath = join14(benchmarkPath, "PRD.md");
  try {
    await stat5(prdPath);
  } catch {
  ...
  try {
    const prdContent = await readFile8(prdPath, "utf-8");
  ...
  const fixturesPath = join14(benchmarkPath, "fixtures");
  try {
    await stat5(fixturesPath);
  } catch {
    result.warnings.push("No fixtures directory");
  }
  const acceptancePath = join14(benchmarkPath, "acceptance");
  try {
    await stat5(acceptancePath);
    const testScripts = ["run-tests.sh", "acceptance.sh", "test.sh"];
    let hasScript = false;
    for (const script of testScripts) {
      try {
        const scriptPath = join14(acceptancePath, script);
        await stat5(scriptPath);
        hasScript = true;
        try {
          await access2(scriptPath, constants.X_OK);
        } catch {
...
// src/cli/index.ts
var benchCommand = new Command7("bench").description("DevPilot Benchmark Suite").addCommand(runCommand).addCommand(reportCommand).addCommand(compareCommand).addCommand(historyCommand).addCommand(listCommand).addCommand(validateCommand);
export {
  benchCommand,
  compareCommand,
  historyCommand,
