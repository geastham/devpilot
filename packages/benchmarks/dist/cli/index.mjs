...
var BaselineExecutor = class {
  ...
  async execute(workspace, collector) {
    ...
    const errors = [];
    try {
      const prdPath = join3(workspace.rootDir, "PRD.md");
...
var DevPilotExecutor = class {
  ...
  async execute(workspace, collector) {
    ...
    const errors = [];
    try {
      const prdPath = join4(workspace.rootDir, "PRD.md");
      ...
      for (const benchmarkRun of benchmarks) {
        ...
        if (benchmarkRun.waveAnalysis) {
          await this.resultsWriter.writeWaveAnalysis(
            join6(runDir, benchmarkRun.benchmarkId),
  ...
  try {
    ...
    switch (type) {
      case "benchmarks": {
        ...
        console.log("\u2550".repeat(50));
        console.log("");
        const entries = await readdir4(benchmarksDir);
        for (const entry of entries) {
...
import { readdir as readdir5, stat as stat4, readFile as readFile8, access as access2, constants } from "fs/promises";
var validateCommand = new Command6("validate").description("Validate benchmark configuration").argument("[benchmarkId]", "Specific benchmark to validate").option("--benchmarks-dir <path>", "Path to benchmarks directory").option("--fix", "Attempt to fix issues").action(async (benchmarkId, options) => {
  ...
  try {
    ...
    try {
      const gtFiles = await readdir5(groundTruthDir);
...
async function validateBenchmark(benchmarksDir, benchmarkId) {
  ...
  try {
    const s = await stat4(benchmarkPath);
    if (!s.isDirectory()) {
