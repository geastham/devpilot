...
__export(index_exports, {
  ...
  BenchmarkIdSchema: () => BenchmarkIdSchema,
  ...
  DevPilotExecutor: () => DevPilotExecutor,
  ...
  ScenarioTypeSchema: () => ScenarioTypeSchema,
  ...
  analyzeWavePlan: () => analyzeWavePlan,
  calculateScore: () => calculateScore,
  compareScenarios: () => compareScenarios,
  createAcceptanceRunner: () => createAcceptanceRunner,
  createBaselineExecutor: () => createBaselineExecutor,
  createBenchmarkRunner: () => createBenchmarkRunner,
  createComparator: () => createComparator,
  createConsoleReporter: () => createConsoleReporter,
  createCostCalculator: () => createCostCalculator,
  createDevPilotExecutor: () => createDevPilotExecutor,
  ...
  createResultsWriter: () => createResultsWriter,
  createScoringEngine: () => createScoringEngine,
  createTimelineRecorder: () => createTimelineRecorder,
  createTimestampPath: () => createTimestampPath,
  createTrendAnalyzer: () => createTrendAnalyzer,
  createWaveAnalyzer: () => createWaveAnalyzer,
  createWorkspaceManager: () => createWorkspaceManager,
  ...
  parseTimestampPath: () => parseTimestampPath,
  sanitizeVersionForPath: () => sanitizeVersionForPath,
  toRunConfig: () => toRunConfig
});
...
function getGitCommit(projectRoot = process.cwd()) {
  try {
    return (0, import_child_process.execSync)("git rev-parse HEAD", {
      cwd: projectRoot,
      encoding: "utf-8",
...
function getGitBranch(projectRoot = process.cwd()) {
  try {
    return (0, import_child_process.execSync)("git rev-parse --abbrev-ref HEAD", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
function getGitTag(projectRoot = process.cwd()) {
  try {
    const tag = (0, import_child_process.execSync)("git describe --exact-match --tags HEAD", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
function getGitDescribe(projectRoot = process.cwd()) {
  try {
    return (0, import_child_process.execSync)("git describe --tags --always", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
function isWorkingTreeDirty(projectRoot = process.cwd()) {
  try {
    const status = (0, import_child_process.execSync)("git status --porcelain", {
      cwd: projectRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
...
var WorkspaceManager = class {
  ...
  async createWorkspace(config) {
    ...
    const workspacePath = (0, import_path3.join)(TEMP_BASE, runId, scenario, benchmarkId);
    await (0, import_promises3.mkdir)(workspacePath, { recursive: true });
    ...
    const prdSource = (0, import_path3.join)(benchmarkPath, "PRD.md");
    const prdDest = (0, import_path3.join)(workspacePath, "PRD.md");
    try {
      await (0, import_promises3.cp)(prdSource, prdDest);
    } catch {
      throw new Error(`PRD.md not found at ${prdSource}`);
    }
    const fixturesSource = (0, import_path3.join)(benchmarkPath, "fixtures");
    const fixturesDest = (0, import_path3.join)(workspacePath, "fixtures");
    try {
      const fixturesStat = await (0, import_promises3.stat)(fixturesSource);
      if (fixturesStat.isDirectory()) {
        await (0, import_promises3.cp)(fixturesSource, fixturesDest, { recursive: true });
      }
    } catch {
    }
    const acceptanceSource = (0, import_path3.join)(benchmarkPath, "acceptance");
    const acceptanceDest = (0, import_path3.join)(workspacePath, "acceptance");
    try {
      const acceptanceStat = await (0, import_promises3.stat)(acceptanceSource);
      if (acceptanceStat.isDirectory()) {
        await (0, import_promises3.cp)(acceptanceSource, acceptanceDest, { recursive: true });
  ...
  async findBenchmarkPath(benchmarksDir, benchmarkId) {
    const exactPath = (0, import_path3.join)(benchmarksDir, benchmarkId);
    try {
      const s = await (0, import_promises3.stat)(exactPath);
      if (s.isDirectory()) return exactPath;
    } catch {
    }
    try {
      const entries = await (0, import_promises3.readdir)(benchmarksDir);
      const prefix = benchmarkId.split("-")[0];
      for (const entry of entries) {
        if (entry.startsWith(`${prefix}-`)) {
          const fullPath = (0, import_path3.join)(benchmarksDir, entry);
  ...
  async makeScriptsExecutable(dir) {
    try {
      const entries = await (0, import_promises3.readdir)(dir);
      for (const entry of entries) {
        if (entry.endsWith(".sh")) {
          const scriptPath = (0, import_path3.join)(dir, entry);
  ...
  async initializePackageJson(workspacePath, benchmarkId, benchmarkPath) {
    const templatePath = (0, import_path3.join)(benchmarkPath, "package.template.json");
    try {
      const template = await (0, import_promises3.readFile)(templatePath, "utf-8");
      await (0, import_promises3.writeFile)((0, import_path3.join)(workspacePath, "package.json"), template);
      return;
    } catch {
    }
    const packageJson = {
    ...
    await (0, import_promises3.writeFile)(
      (0, import_path3.join)(workspacePath, "package.json"),
  ...
  async cleanupWorkspace(workspaceId, options = {}) {
    ...
    try {
      await (0, import_promises3.rm)(info.path, { recursive: true, force: true });
    } catch {
    }
  ...
  async archiveWorkspace(workspacePath, archivePath) {
    await (0, import_promises3.mkdir)((0, import_path3.dirname)(archivePath), { recursive: true });
    try {
      await tar.create(
        {
          gzip: true,
          file: archivePath,
          cwd: (0, import_path3.dirname)(workspacePath)
        },
      ...
      await this.processManager.spawn("tar", [
        ...
        archivePath,
        "-C",
        (0, import_path3.dirname)(workspacePath),
        workspacePath.split("/").pop()
      ]);
    }
  }
...
var AcceptanceRunner = class {
  ...
  async findTestScript(workspacePath) {
    ...
    for (const candidate of candidates) {
      const fullPath = (0, import_path4.join)(workspacePath, candidate);
      try {
  ...
  async needsServer(workspacePath, testScript) {
    try {
      const content = await (0, import_promises4.readFile)(testScript, "utf-8");
  ...
  async startServer(workspacePath) {
    ...
    for (const { cmd, args } of serverCommands) {
      try {
        if (cmd === "node") {
          await (0, import_promises4.stat)((0, import_path4.join)(workspacePath, args[0]));
        }
...
var BaselineExecutor = class {
  ...
  async execute(workspace, collector) {
    ...
    try {
      const prdPath = (0, import_path5.join)(workspace.path, "PRD.md");
      const prdContent = await (0, import_promises5.readFile)(prdPath, "utf-8");
...
var DevPilotExecutor = class {
  ...
  async execute(workspace, collector) {
    ...
    const errors = [];
    try {
      const prdPath = (0, import_path6.join)(workspace.path, "PRD.md");
      const prdContent = await (0, import_promises6.readFile)(prdPath, "utf-8");
      collector.getTimeline().recordEvent({
        type: "run_start",
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
      });
...
var BenchmarkRunner = class {
  ...
  async run(runConfig) {
    ...
    try {
      ...
      await this.resultsWriter.updateLatestSymlink(
        (0, import_path7.join)(this.config.resultsDir, version.version, startTime.toISOString().replace(/[:.]/g, "-"))
      );
      return run;
    } finally {
  ...
  async runBenchmark(runId, benchmarkId, runConfig, groundTruth) {
    ...
    if (runConfig.scenarios.includes("baseline")) {
      ...
      await this.workspaceManager.cleanupWorkspace(workspaceId, {
        ...
        archivePath: (0, import_path7.join)(
  ...
  async loadGroundTruths(benchmarkIds) {
    ...
    const groundTruthDir = (0, import_path7.join)(this.config.benchmarksDir, "ground-truth");
    for (const benchmarkId of benchmarkIds) {
      try {
        const exactPath = (0, import_path7.join)(groundTruthDir, `${benchmarkId}.json`);
        const content = await (0, import_promises7.readFile)(exactPath, "utf-8");
        ...
        try {
          ...
          if (matchingFile) {
            const content = await (0, import_promises7.readFile)((0, import_path7.join)(groundTruthDir, matchingFile), "utf-8");
            groundTruths.set(benchmarkId, JSON.parse(content));
          }
        } catch {
        }
