...
var WikiCompiler = class {
  ...
  async ingest(content, sourceType, title, origin) {
    ...
    const existing = await db2.select().from(wikiSources).where(eq(wikiSources.contentHash, contentHash)).limit(1);
    ...
    for (const article of articles) {
      const existingArticle = await db2.select().from(wikiArticles).where(eq(wikiArticles.slug, article.slug)).limit(1);
  ...
  async query(question) {
    const db2 = getDatabase();
    const allArticles = await db2.select().from(wikiArticles).where(eq(wikiArticles.status, "active"));
...
${s.article.content}`
    ...
    if (parsed.suggestedNewArticle) {
      ...
      const existingArticle = await db2.select().from(wikiArticles).where(eq(wikiArticles.slug, newArticle.slug)).limit(1);
      if (existingArticle.length === 0) {
        await db2.insert(wikiArticles).values({
...
${a.content}`
    ...
    for (const finding of parsed.findings) {
      if (finding.type === "stale") {
        await db2.update(wikiArticles).set({ status: "stale", updatedAt: /* @__PURE__ */ new Date() }).where(eq(wikiArticles.slug, finding.articleSlug));
  ...
  async getArticle(slug) {
    const db2 = getDatabase();
    const results = await db2.select().from(wikiArticles).where(eq(wikiArticles.slug, slug)).limit(1);
...
`;
    ...
    for (const [category, catArticles] of Object.entries(byCategory)) {
      ...
      for (const entry of catArticles) {
        const fullArticle = await db2.select().from(wikiArticles).where(eq(wikiArticles.slug, entry.slug)).limit(1);
