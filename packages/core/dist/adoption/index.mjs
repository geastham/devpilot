...
function loadOwnedSessionIds(path) {
  ...
  try {
    if (!existsSync2(file)) return /* @__PURE__ */ new Set();
...
async function summarizeSession(observation, touchedPaths, options = {}) {
  ...
  try {
    const client = options.clientFactory ? options.clientFactory(apiKey) : new Anthropic({ apiKey, timeout: options.timeoutMs ?? REQUEST_TIMEOUT_MS });
